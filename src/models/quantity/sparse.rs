...
impl QuantityLp {
    ...
    fn clear(&mut self) -> grb::Result<()> {
        ...
        time!("clear", {
            self.model = Model::new(&format!("quantities-sparse"))?;
            // Disable output logging.
    ...
    pub fn configure(
        ...
        time!("configure", {
            ...
            let mut t_n =
                vec![[0, t - 1].into_iter().collect::<HashSet<_>>(); problem.nodes().len()];
            let mut t_v = problem
                .vessels()
                .iter()
            ...
            /* // Note: t_v will be constructed in order of increasing t with this, since it is
                   ...
                   let mut active = std::fs::File::create("active.txt").unwrap();
            */
            for (v, nodes) in Self::active(solution) {
                /*  writeln!(active, "vessel = {v}"); */
            ...
            for (v, timeline) in t_v.iter().enumerate() {
                ...
                assert!(
                    timeline[0] == problem.vessels()[v].available_from(),
                    "{} != {}",
                    timeline[0],
                    problem.vessels()[v].available_from()
                );
    ...
    pub fn solve(&mut self) -> grb::Result<&Variables> {
        ...
        match self.model.status()? {
            ...
            status => {
                ...
                self.model.write(&format!("model.lp"));
                let mut file = std::fs::File::create(format!("solution.txt")).unwrap();
                write!(file, "{:#?}", solution.to_vec());
